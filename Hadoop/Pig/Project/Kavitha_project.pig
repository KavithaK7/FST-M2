...
inputFile1 = LOAD 'hdfs:///user/kavithaK/inputs/episode1_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile2 = LOAD 'hdfs:///user/kavithaK/inputs/episode2_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile3 = LOAD 'hdfs:///user/kavithaK/inputs/episode3_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
...
STORE ordered INTO 'hdfs:///user/kavithaK/output' USING PigStorage('\t');
